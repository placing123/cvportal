...
class AdminController extends CI_Controller
{
	...
	public function register_store()
	{
		...
		$send_mail = $this->send_mail($customer_id, $password, $pdf, $email);
		...
		if ($send_mail == true) {
			...
			$this->session->set_flashdata('success', 'Registered Successfully ID is ' . $customer_id . ' And passowrd is ' . $password . ' /Register Email not send');
		}
	...
	function send_mail2($customer_id, $password, $pdf, $email)
	{
		...
		$fromemail = "support@cvportals.com	";
		$toemail = $email;
	...
	function send_mail($customer_id, $password, $pdf, $email)
	{
		...
		try {
			...
			$mail->Username   = 'support@cvportals.com';                     //SMTP username
			$mail->Password   = 'cvportal@2123';
			...
			//Recipients
			$mail->setFrom('support@cvportals.com', 'onlineplacmenet');
			$mail->addAddress($email);     //Add a recipient
			$audio_file = '/home/ltx8i4fswi8y/cvportals.com/admin_assets/audio_demo.mp3';
			$attactfile = '/home/ltx8i4fswi8y/cvportals.com/admin_assets/pdf/' . $pdf;
	...
	function activate_agreement()
	{
		...
		for ($i = 0; $i < $forms; $i++) {
			$insert['resume_id'] = rand(1, $result);
	...
	public function send_mail_withmailer($email, $sendfrom, $mesg, $subject)
	{
		...
		try {
			...
			//$mail->SMTPDebug = SMTP::DEBUG_SERVER;                      //Enable verbose debug output
			$mail->isSMTP();                                            //Send using SMTP
			$mail->Host       = 'mail.theinfoservices.in';                     //Set the SMTP server to send through
			$mail->SMTPAuth   = true;                                   //Enable SMTP authentication

			$mail->Username   = 'legal@cvportals.com';                     //SMTP username
			$mail->Password   = '?V]9bgJHhMHd';
			...
			$mail->setFrom($sendfrom, 'onlineplacement');
			$mail->addAddress($email);     //Add a recipient
			$mail->isHTML(true);                                  //Set email format to HTML
	...
	function customer_qc()
	{
		...
		$data['cust_rec'] = $res;
		$data['action'] = $attributes; //'form_approve';
		$this->load->view('admin/dashboard', $data);
	}
